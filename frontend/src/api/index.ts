...
export interface IndicatorEvalItem {
  indicatorId: string
  name: string
  rawValue: number
  boolValue: boolean
...
export interface StockEvalResult {
  ...
  totalScore: number
  indicators: IndicatorEvalItem[]
  rank: number
}

export interface StrategyEvalResponse {
  total: number
  page: number
  pageSize: number
  items: StockEvalResult[]
  costMs: number
}

// 获取策略列表
export async function getStrategyList(): Promise<ApiResponse<Strategy[]>> {
  return request<Strategy[]>('/strategy/list')
}

// 获取单个策略
export async function getStrategy(id: string): Promise<ApiResponse<Strategy>> {
  return request<Strategy>(`/strategy/get/${id}`)
}

// 创建策略
export async function createStrategy(s: Partial<Strategy>): Promise<ApiResponse<Strategy>> {
  return request<Strategy>('/strategy/create', { method: 'POST', body: JSON.stringify(s) })
}

// 更新策略
export async function updateStrategy(id: string, s: Partial<Strategy>): Promise<ApiResponse<Strategy>> {
  return request<Strategy>(`/strategy/update/${id}`, { method: 'POST', body: JSON.stringify(s) })
}

// 删除策略
export async function deleteStrategy(id: string): Promise<ApiResponse<null>> {
  return request<null>(`/strategy/delete/${id}`, { method: 'POST' })
}

// 设为默认
export async function setDefaultStrategy(id: string): Promise<ApiResponse<null>> {
  return request<null>(`/strategy/set-default/${id}`, { method: 'POST' })
}

// 复制策略
export async function copyStrategy(id: string): Promise<ApiResponse<Strategy>> {
  return request<Strategy>(`/strategy/copy/${id}`, { method: 'POST' })
}

// 获取默认策略
export async function getDefaultStrategy(): Promise<ApiResponse<Strategy>> {
  return request<Strategy>('/strategy/default')
}
...
export async function evaluateStrategy(payload: {
  ...
  page?: number
  pageSize?: number
}, timeoutMs = 120000): Promise<ApiResponse<StrategyEvalResponse>> {
  return request<StrategyEvalResponse>('/strategy/evaluate', {
    method: 'POST',
...
// 获取指标库
export async function getIndicatorLib(): Promise<ApiResponse<IndicatorDefinition[]>> {
  return request<IndicatorDefinition[]>('/strategy/indicators')
}
