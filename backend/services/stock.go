...
func (s *StockService) GetMarketIndices() (*models.MarketData, error) {
	codes := []string{"sh000001", "sz399001", "sz399006", "sh000688", "bj899050"}
		...
		for _, q := range quotes {
			for suffix, key := range keyMap {
				if strings.HasSuffix(q.Code, suffix) {
					amplitude := 0.0
					if q.PrevClose > 0 {
						amplitude = ((q.High - q.Low) / q.PrevClose) * 100
					}
...
func (s *StockService) DiagnoseStock(code string) (*models.DiagnoseResult, error) {
	...
	if n < 10 {
		r.Score = r.TrendScore + r.MomentumScore + r.VolumeScore + r.TechScore
		r.Recommendation = "hold"
		r.RecLabel = "数据不足"
		r.Analysis = fmt.Sprintf("%s 数据不足（仅%d根K线），无法完整诊断", quote.Name, n)
		return r, nil
	}
	...
	if macdData != nil {
		r.MacdDif = macdData.Dif
		r.MacdDea = macdData.Dea
		r.MacdHist = macdData.Hist
		r.MacdSignal = macdData.Signal
		r.MacdStatus = macdData.Status
	}

	// ── BOLL ──
	bollData := calcBOLL(klines)
	if bollData != nil {
		r.BollUpper = bollData.Upper
		r.BollMiddle = bollData.Middle
		r.BollLower = bollData.Lower
		if bollData.Upper-bollData.Lower > 0 {
			pos := (last - bollData.Lower) / (bollData.Upper - bollData.Lower) * 100
			if pos >= 95 {
				r.BollPos = "突破上轨"
			} else if pos >= 70 {
				r.BollPos = "上轨附近"
	...
	if len(ma10) > 0 {
		r.Ma10 = ma10[len(ma10)-1]
	}
	if len(ma20) > 0 {
		r.Ma20 = ma20[len(ma20)-1]
	}
	if len(ma60) > 0 {
		r.Ma60 = ma60[len(ma60)-1]
	}
	// 均线排列判定
	aboveMa5 := last > r.Ma5
	aboveMa10 := last > r.Ma10
	aboveMa20 := last > r.Ma20
	aboveMa60 := r.Ma60 > 0 && last > r.Ma60
	if aboveMa5 && aboveMa10 && aboveMa20 && aboveMa60 {
		r.MaAlignment = "多头排列"
	} else if aboveMa5 && aboveMa10 {
		r.MaAlignment = "短线偏多"
	} else if !aboveMa5 && !aboveMa10 {
		r.MaAlignment = "空头排列"
	} else {
	...
	if n >= 9 {
		...
		r.KdjK = math.Round(k*100) / 100
		r.KdjD = math.Round(d*100) / 100
		r.KdjJ = math.Round((3*k-2*d)*100) / 100
	}

	// ── 量比 ──
	vol5 := sma(volumes, 5)
	if len(vol5) > 0 && vol5[len(vol5)-1] > 0 && n >= 2 {
		// K线量是股，Sina量是手，统一到股
		klineVol := volumes[n-1]
		sinaVol := quote.Volume * 100
		// 优先用K线量（更连续）
		if klineVol > 0 {
			r.VolumeRatio = math.Round(klineVol/vol5[len(vol5)-1]*100) / 100
		} else if sinaVol > 0 {
			r.VolumeRatio = math.Round(sinaVol/vol5[len(vol5)-1]*100) / 100
		}
	}

	// ── 箱体检测 ──
	if n >= 40 {
		boxTop, boxBot, _, _ := detectBox(highs, lows, 40)
		if boxTop > boxBot && boxBot > 0 {
			r.BoxTop = math.Round(boxTop*100) / 100
			r.BoxBottom = math.Round(boxBot*100) / 100
			if boxTop > boxBot {
				r.BoxPosPct = math.Round((last-boxBot)/(boxTop-boxBot)*1000) / 10
			}
		}
	}
	...
	if n >= 10 {
		...
		r.Support = math.Round(sup*100) / 100
		r.Resistance = math.Round(res*100) / 100
	}

	// ── 四维统一评分 ──
	scoreMap := s.GetStockScores([]string{code})
	if ss, ok := scoreMap[code]; ok && ss != nil {
		r.Score = math.Round(ss.TotalScore*10) / 10
		r.TrendScore = ss.TrendScore
		r.MomentumScore = ss.MomentumScore
		r.VolumeScore = ss.VolumeScore
		r.TechScore = ss.TechScore
	}

	// ── 亮点提取 ──
	var highlights []string
	if r.MacdSignal == "水上金叉" || r.MacdSignal == "水下金叉" {
		highlights = append(highlights, "MACD金叉")
	}
	if macdData != nil && macdData.AxisPosition == "水上" {
		highlights = append(highlights, "MACD水上")
	}
	if r.BollPos == "突破上轨" {
		highlights = append(highlights, "突破布林上轨")
	}
	if r.MaAlignment == "多头排列" {
		highlights = append(highlights, "均线多头")
	}
	if r.Rsi > 70 {
		highlights = append(highlights, "RSI超买")
	} else if r.Rsi < 30 && r.Rsi > 0 {
		highlights = append(highlights, "RSI超卖")
	}
	if r.VolumeRatio > 1.5 {
		highlights = append(highlights, "放量上攻")
	} else if r.VolumeRatio < 0.5 && r.VolumeRatio > 0 {
		highlights = append(highlights, "缩量调整")
	}
	if r.BoxBottom > 0 {
		highlights = append(highlights, fmt.Sprintf("箱体形态(%.1f-%.1f)", r.BoxBottom, r.BoxTop))
		if r.BoxPosPct <= 20 {
			highlights = append(highlights, "箱底买入区")
		} else if r.BoxPosPct >= 80 {
			highlights = append(highlights, "箱顶压力区")
		}
	}
	if r.ChangePercent >= 5 {
		highlights = append(highlights, "强势上攻")
	} else if r.ChangePercent <= -5 {
		highlights = append(highlights, "大幅回调")
	}
	...
	// ── 分析结论 ──
	parts := []string{
		fmt.Sprintf("综合评分 %.1f 分", r.Score),
		fmt.Sprintf("MACD %s", ifStr(r.MacdSignal == "", "未知", r.MacdSignal)),
		fmt.Sprintf("BOLL %s", ifStr(r.BollPos == "", "未知", r.BollPos)),
	}
	if r.BoxBottom > 0 {
		parts = append(parts, fmt.Sprintf("箱体范围 %.2f~%.2f", r.BoxBottom, r.BoxTop))
	}
	if len(highlights) > 0 {
		parts = append(parts, strings.Join(highlights[:min(3, len(highlights))], "、"))
	}
	r.Analysis = strings.Join(parts, " · ")
...
func (s *StockService) GetStockDetail(code string) (*models.StockTechnicalDetail, error) {
	...
	klines, kerr := s.fetchKLineData(code, 120)
	if kerr != nil {
		log.Printf("[KLine] 获取K线失败 %s: %v", code, kerr)
		klines = []models.KLineData{}
	}
	macdData := calcMACD(klines)
	bollData := calcBOLL(klines)
...
func (s *StockService) ScanWatchList() (*models.WatchScanResult, error) {
	...
	// 统一评分：从缓存或实时计算
	scoreMap := s.GetStockScores(codes)
...
func (s *StockService) GetStockScores(codes []string) map[string]*models.StockScore {
	...
	for _, code := range codes {
		if cached, ok := cacheMap[code]; ok {
			...
			// 缓存未命中：实时计算统一评分（与 ScanAllAShares 完全一致的算法）
			if fb := s.computeStockScoreFallback(code); fb != nil {
				result[code] = fb
			}
		}
	}
...
func (s *StockService) syncDualEngineScoresToCache(stocks []models.DualEngineStock) {
	...
	for _, stock := range stocks {
		if idx, ok := existingMap[stock.Code]; ok {
			// 已存在：只更新价格/换手/量比等行情数据，保留已有的四维评分
			s.allStockCache[idx].Price = stock.Price
			s.allStockCache[idx].ChangePercent = stock.ChangePercent
			s.allStockCache[idx].TurnoverRate = stock.TurnoverRate
			s.allStockCache[idx].VolumeRatio = stock.VolumeRatio
			s.allStockCache[idx].MacdSignal = stock.MacdSignal
			s.allStockCache[idx].BollPosition = stock.BollPosition
			s.allStockCache[idx].IsGoldenCross = stock.IsGoldenCross
			s.allStockCache[idx].IsAboveWater = stock.IsAboveWater
			s.allStockCache[idx].Highlights = stock.Score.Highlights
		} else {
			// 新股票：追加到缓存，评分使用双引擎结果
...
func (s *StockService) fetchKLineData(code string, count int) ([]models.KLineData, error) {
	...
	url := fmt.Sprintf("https://quotes.sina.cn/cn/api/json_v2.php/CN_MarketData.getKLineData?symbol=%s&scale=240&ma=no&datalen=%d", symbol, count)
	...
	// 新浪K线返回JSON数组: [{"day":"2024-01-15","open":"3200.45","high":"3210.00","low":"3185.00","close":"3205.00","volume":"385000000"},...]
	...
	for _, item := range raw {
		...
		close_, _ := strconv.ParseFloat(item.Close, 64)
...
// ── K-line cache for strategy evaluation ──
...
func (s *StockService) fetchKLineDataWithCache(code string, days int) []models.KLineData {
	klineCacheMu.RLock()
	entry, ok := klineCache[code]
	klineCacheMu.RUnlock()

	if ok && time.Since(entry.FetchedAt) < 5*time.Minute && len(entry.Data) >= days {
		return entry.Data
	}

	klines, err := s.fetchKLineData(code, days)
	...
	klineCacheMu.Lock()
	klineCache[code] = &klineCacheEntry{Data: klines, FetchedAt: time.Now()}
	klineCacheMu.Unlock()
	return klines
}

// GetAllKLineData returns K-line data for the top N cached stocks
func (s *StockService) GetAllKLineData() (map[string][]models.KLineData, []models.SinaQuote) {
	s.allStockCacheMu.RLock()
	cache := s.allStockCache
	s.allStockCacheMu.RUnlock()
	...
	// Limit to top 300 stocks with highest scores to avoid timeout
	maxStocks := 300
	if len(cache) < maxStocks {
		maxStocks = len(cache)
	}
	topN := cache[:maxStocks]

	var quotes []models.SinaQuote
	for _, item := range topN {
		quotes = append(quotes, models.SinaQuote{
			Code:          item.Code,
			Name:          item.Name,
			Price:         item.Price,
	...
	for _, item := range topN {
		...
		go func(code string) {
			...
			klines := s.fetchKLineDataWithCache(code, 60)
			if len(klines) > 0 {
				mu.Lock()
